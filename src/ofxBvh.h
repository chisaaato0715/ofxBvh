...
public:
	...
	inline 	void setMatrix(ofMatrix4x4 mat) { matrix = mat; }
	inline 	void setGlobalMatrix(ofMatrix4x4 mat) { global_matrix = mat; }
...
public:
	...
	void newBvhdata();                                                  //新しいファイルを出力する関数
	void getSpace(int z, stringstream& sstr);                           //z個スペースをつくる関数
	void getjoint(const ofxBvhJoint *joint, int z, stringstream& sstr); //HIERARCYをつくる関数

	void updateIK(ofxBvhJoint *joint);                  //IK計算の関数
	void updateFK(ofxBvhJoint *joint);                                  //FK計算の関数
	int selectJoint(ofVec3f target);

	ofVec3f target,screenTarget;
	int selectjoint;

   //===============================================================================

	vector<ofxBvhJoint*> joints;                                        //protectedから移動
